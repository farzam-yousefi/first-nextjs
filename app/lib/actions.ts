...
export async function createInvoice(prevState: State, formData: FormData) {
  ...
  if (!validatedFields.success) {
    let customer: CustomerField[] =[{id:"" ,name:""}]
//console.log(formData);
//return
if (formData.get("customerId")) {
    const id=formData?.get("customerId") as string
    console.log("id isssssssss" ,id)
  customer = await fetchCustomerById(id);
  
}
    console.log(customer,"KKKKKK");
    return {
      errors: validatedFields.error.flatten().fieldErrors,
      message: "Missing Fields. Failed to Create Invoice.",
      defaultValues :{
        customerId:formData.get("customerId")  as string,
        customerName:customer[0].name as string,
